...
mydict = {}
base = np.loadtxt("dualmotorrobustpidone0.9.txt")
x = []
y = []
y2 = []
for i in range(len(arr)):
    y2.append(base[i][1])
    data = pd.read_csv("dualmotorrobustone0.9other" + str(arr[i]) + ".txt", header=None)
    values = []
...
plt.xlabel("Resistance Fault of Motor 5")
plt.ylabel("Sum of Absolute X and Y Error (m)")
plt.title("Robustness of Controller and Comparison to Nominal PID Controller With Motor 6 at 4x Fault and Motor 5 Varying")
plt.legend()
plt.show()
