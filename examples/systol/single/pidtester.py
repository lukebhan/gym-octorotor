...
if __name__ == "__main__":
    ...
    f = open("pidEPATHXTrajMotor3Error.txt", "w")
    f2 = open("pidEPATHYTrajMotor3Error.txt", "w")
    ...
    for i in range(1):
        ...
        model = PPO.load("./rlMotor3Part3/model_1000_steps")
        act = model.predict([obs])[0]
        obs, rew, done, err = env.step(act[0])
        for k in range(len(err["xerror"])):
            f.write(str(err["xerror"][k]) + "\n")
            f2.write(str(err["yerror"][k]) + "\n")
