...
if __name__ == "__main__":
    ...
    log_dir = "rlMotor5Part4"
    os.makedirs(log_dir, exist_ok=True)

    env = gym.make('octorotor-v0', OctorotorParams=OctorotorParams)
    env = Monitor(env, log_dir)
    #policy_kwargs = dict( net_arch=dict(pi=[64], qf=[64]))
    checkpoint_callback = CheckpointCallback(save_freq=500, save_path='./rlMotor5Part4/',
                                             name_prefix='model')
    model = PPO.load("./rlMotor5Part3/model_1000_steps", env=env,verbose=1)
