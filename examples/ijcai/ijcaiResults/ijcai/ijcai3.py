...
if __name__ == "__main__":
    ...
    log_dir = "ijcaisac4Part3"
    os.makedirs(log_dir, exist_ok=True)

    env = gym.make('octorotor-v0', OctorotorParams=OctorotorParams)
    env = Monitor(env, log_dir)
    #policy_kwargs = dict( net_arch=dict(pi=[64], qf=[64]))
    checkpoint_callback = CheckpointCallback(save_freq=500, save_path='./ijcaisac4Part3',
                                             name_prefix='model')
    #model = PPO.load("./rlMotor3Part2/model_1000_steps", env=env,verbose=1)
    #model = SAC("MlpPolicy", env=env, verbose=1)
    model = SAC.load("./ijcaisac4Part2/model_1000_steps", env, verbose=1)
