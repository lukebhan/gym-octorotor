...
if __name__ == "__main__":
    ...
    log_dir = "ijcaippo4Part2"
    os.makedirs(log_dir, exist_ok=True)

    env = gym.make('octorotor-v0', OctorotorParams=OctorotorParams)
    env = Monitor(env, log_dir)
    #policy_kwargs = dict( net_arch=dict(pi=[64], qf=[64]))
    checkpoint_callback = CheckpointCallback(save_freq=500, save_path='./ijcaippo4Part2',
                                             name_prefix='model')
    model = PPO.load("./ijcaippo4/model_1000_steps", env=env,verbose=1)
    #model = PPO("MlpPolicy", env=env, verbose=1, n_steps=5)
    #model = SAC.load("./ijcaisac/model_1000_steps", env, verbose=1)
